...
const { encryptId, decryptId } = require("../middleware/cryptic");
    ...
    try {
      ...
      if (err.name === "TokenExpiredError") {
        try {
          ...
          validToken = jwt.sign({ id: admindId }, process.env.ACCESS_TOKEN, {
            expiresIn: "15m",
          });
...
AdminAuth.get("/admin/find-admin", verify, async (req, res) => {
  try {
    const admin = await Admin.findById(req.admin).select(
      "-adminKey -refreshToken -adminToken -_id"
    );
    if (!admin)
      return res.status(400).json({ msg: "Admin Does Not Does Exist." });
